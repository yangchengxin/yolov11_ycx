...
void serialize_engine(std::string &wts_name, std::string &engine_name, float &gd, float &gw, int &max_channels, std::string &type) {
...
void prepare_buffer(ICudaEngine *engine, float **input_buffer_device, float **output_buffer_device,
    ...
    /* 给输入在gpu上开辟内存，因为是数组内的一个元素，用双重指针指向 */
    CUDA_CHECK(cudaMalloc((void **) input_buffer_device, kBatchSize * 3 * kInputH * kInputW * sizeof(float)));
    /* 给输出在gpu上开辟内存，因为是数组内的一个元素，用双重指针指向 */
...
int main(int argc, char **argv) {
    ...
    std::string wts_name = R"(E:\DeepLearning\repository\yolov11_ycx\YOLO11-series-TensorRT8\objects\yolo11n.wts)";
    std::string engine_name = R"(E:\DeepLearning\YOLOv11\YOLO11-series-TensorRT8\build\Debug\yolo11n.fp32.trt)";
    std::string img_dir = "E:\\DeepLearning\\YOLOv5\\data\\images";
