...
def get_rect_obb(img, bbox):
    ...
    if r_h > r_w:
        l = l
        r = r
        t = t - (INPUT_H - r_w * img.shape[0]) / 2
        b = b - (INPUT_H - r_w * img.shape[0]) / 2
        l = l / r_w
        r = r / r_w
        t = t / r_w
        b = b / r_w
    else:
        l = l - (INPUT_W - r_h * img.shape[1]) / 2
        r = r - (INPUT_W - r_h * img.shape[1]) / 2
        t = t
        b = b
        l = l / r_h
        r = r / r_h
    ...
    l = max(0.0, l)
    t = max(0.0, t)
    width = max(0, min(int(round(r - l)), img.shape[1] - int(round(l))))
    height = max(0, min(int(round(b - t)), img.shape[0] - int(round(t))))

    return [int(round(l)), int(round(t)), width, height]
