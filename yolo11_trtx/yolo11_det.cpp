...
int main(int argc, char **argv) {
    ...
    /* 直接运行 */
    engine_name = R"(D:\deep_learning\YOLO11-series-TensorRT8\build\Debug\yolo11n.engine)";
    img_dir = R"(D:\deep_learning\YOLO11-series-TensorRT8\test)";
    cuda_post_process = "c";

    // Create a model using the API directly and serialize it to a file
    //wts_name = R"(yolo11n.wts)";
    //if (!wts_name.empty()) {
    ...
    std::vector<cv::Mat> zeroBatch = { zero };
    cuda_batch_preprocess(zeroBatch, device_buffers[0], kInputW, kInputH, stream);
    for (int i = 0; i < 10; i++)
    {
